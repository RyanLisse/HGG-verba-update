import type { FormattedDocument, VerbaVector } from '@/app/types';
...
export function vectorToRGB(
  ...
  // Scale normalized values to 0-255 (match previous channel mapping: Z->R, X->G, Y->B)
  const r = Math.floor(normalizedZ * 255);
  const g = Math.floor(normalizedX * 255);
  const b = Math.floor(normalizedY * 255);

  return [r, g, b];
}

// Backward compatibility: previous code imported vectorToColor and expected a THREE.Color.
// We now return a CSS rgb() string which works with R3F's color prop and material.set().
export function vectorToColor(
  ...
  const [r, g, b] = vectorToRGB(vector, minX, maxX, minY, maxY, minZ, maxZ);
  return `rgb(${r},${g},${b})`;
}
