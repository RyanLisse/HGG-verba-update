    ...
    async def chunk(
        ...
        for document in documents:
            ...
            for i, chunk in enumerate(text_splitter.split_text(document.content)):
                ...
                # append title and page content (should only be one header as we are
                # splitting at header so index at 0), if a header is found
                ...
                document.chunks.append(
                    Chunk(
                        content=chunk_text,
                        chunk_id=str(i),
                        start_i=0,  # not implemented as HTML text splitter
                        # changes the actual document (removes tags)
