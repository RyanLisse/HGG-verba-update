...
class Chunker(VerbaComponent):
    ...
    async def chunk(
        ...
        @parameter: embedder : Embedding | None - (Optional) Selected Embedder
        if the Chunker requires vectorization
...
class Generator(VerbaComponent):
    ...
    def __init__(self):
        ...
        default_prompt = (
            "You are Verba, a chatbot for Retrieval Augmented Generation (RAG). "
            "You will receive a user query and context pieces that have a semantic "
            "similarity to that query. Please answer these user queries only with "
            "the provided context. Mention documents you used from the context if "
            "you use them to reduce hallucination. If the provided documentation "
            "does not provide enough information, say so. If the user asks "
            "questions about you as a chatbot specifially, answer them naturally. "
            "If the answer requires code examples encapsulate them with "
    ...
    def prepare_messages(
        ...
        Generation chatbot system, including system instructions, previous
        conversation, and a new user query with context.

        @parameter queries: A list of strings representing the user queries to
        be answered.
        @parameter context: A list of strings representing the context
        information provided for the queries.
        @parameter conversation: A list of previous conversation messages that
        include the role and content.

        @returns A list or of message dictionaries or whole prompts formatted
        for the chatbot. This includes an initial system message, the previous
        conversation messages, and the new user query encapsulated with the
        provided context.

        Each message in the list is a dictionary with 'role' and 'content'
        keys, where 'role' is either 'system' or 'user', and 'content' contains
        the relevant text. This will depend on the LLM used.
        """
        raise NotImplementedError(
...
class ModelFetcher(ABC):
    """Abstract base for fetching available models from APIs."""
...
class AsyncSafeModelProvider(ABC):
    """Base class for components that need async-safe model fetching."""
    
    def __init__(self, model_fetcher: Optional[ModelFetcher] = None):
        self.model_fetcher = model_fetcher
    
    @abstractmethod
    async def get_models_async(self, token: str, url: str) -> List[str]:
        """Get models in an async-safe way."""
        ...
    
    def get_models_sync(self, token: str, url: str) -> List[str]:
        """Get models in sync context - should use async internally if possible."""
        import asyncio
        try:
            # Try to get current event loop
            loop = asyncio.get_running_loop()
            # If we're in an event loop, we need to handle this differently
            # This is where we'll implement proper async task handling
            return self._get_models_with_running_loop(token, url, loop)
        except RuntimeError:
            # No event loop running, safe to use asyncio.run
            return asyncio.run(self.get_models_async(token, url))
    
    def _get_models_with_running_loop(self, token: str, url: str, loop: asyncio.AbstractEventLoop) -> List[str]:
        """Handle model fetching when event loop is already running."""
        # For now, return defaults to avoid blocking
        # In the future, this could schedule a task or use thread pool
